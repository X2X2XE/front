...
public class FileHandler {
	...
	public List<FileVO> getFileList(MultipartFile[] files) {
		...
		today = today.replace("-", File.separator); // 2002\07\22(win), 2022/07/22(linux,mac)
		...
		for (MultipartFile file : files) {
			...
			String onlyFileName = originalFileName.substring(
					originalFileName.lastIndexOf("\\")+1); // 경보제거 후 순수파일.확장자 이름만
			...
			try {
				...
				if(isImageFile(storeFile)) { // 이미지 파일이면 썸네일 제작
					fvo.setFileType(1);
	...
	public FileVO getFile(MultipartFile files) {
		LocalDate date = LocalDate.now(); 
		String today = date.toString(); // 2022-07-22
		today = today.replace("-", File.separator); // 2002\07\22(win), 2022/07/22(linux,mac)
		...
		if(!folder.exists()) {
			...
			try {
				files.transferTo(storeFile); // 원본객체를 복사하여 저장하는 방식
				if(isImageFile(storeFile)) { // 이미지 파일이면 썸네일 제작
					fvo.setFileType(1);
					File thumbNail = new File(folder, 
							uuid.toString() + "_th_" + onlyFileName);
