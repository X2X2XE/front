  ...
  if (cmtInputObj.value == null || cmtInputObj.value == '') {
    ...
    postCommentToServer(cmtData).then(result => {
      if (parseInt(result)) {
        alert('댓글 등록 성공~');
        cmtInputObj.value = "";
        getCommentList(document.getElementById('bnoVal').innerText, cmwriter);
      }
    });
  }
});
async function spreadCommentFromServer(bnoVal, pageNo = 1) {
  try {
    const resp = await fetch('/answer/' + bnoVal + "/" + pageNo); // RESTful
...
function getCommentList(bnoVal, cmwriter, pageNo=1) {
...
document.addEventListener('click', (e) => {
  ...
  if(e.target.id == 'moreBtn'){
    ...
    getCommentList(bnoVal, cmwriter,parseInt(page)+1);
