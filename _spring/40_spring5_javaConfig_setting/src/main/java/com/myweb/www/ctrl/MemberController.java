...
public class MemberController {
	...
	public String register(MemberVO mvo) {
		log.info(">>> register-mvo : {}", mvo.toString());
	...
	public void list(Model model, ModelAndView mav) {
	...
	@GetMapping("/modifiy")
	public void modifiy(Model model, @RequestParam("email") String email) {
		MemberVO mvo = msv.detail(email);
		model.addAttribute("mvo", mvo);
	}
	@PostMapping("/modifiy")
	public String modifiy(MemberVO mvo) {
		msv.modifiy(mvo);
	...
	public String login(MemberVO mvo, HttpSession ses, RedirectAttributes rttr) {
		int lastLog = msv.modlog(mvo.getEmail());
		MemberVO sesMvo = msv.login(mvo);
		log.info(">>> member login > lastLogin update : {}", lastLog > 0 ? "OK" : "FAIL");
		if(sesMvo != null) {
			log.info(">>> member login - Ok");
			ses.setAttribute("ses", sesMvo);
			ses.setMaxInactiveInterval(600); // 10 Min
