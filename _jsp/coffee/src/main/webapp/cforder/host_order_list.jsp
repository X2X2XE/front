              ...
              <c:choose>
                ...
                <c:when test="${cfovo.made eq 4 }">
                  <input type="button" value="매장에서 주문 취소" class="btn btn-danger" disabled>
						...
						<c:choose>
						  <c:when test="${cfovo.made eq 1 }">
						    <a href="/cfo/host_order_finish?ono=${cfovo.ono }" class="btn btn-info">주문완료</a>
						    <a href="/cfo/host_order_cancel?ono=${cfovo.ono }" class="btn btn-danger">주문취소</a>
						  </c:when>
						  <c:when test="${cfovo.made ne 1 }">
						    <button type="button"
							  class="btn btn-warning" disabled>주문완료</button>
							</c:when>
            </c:choose>							
					 </td>
					</form>
