...
public class GameStarter {
	...
	public static void main(String[] args) {
		...
		while(ag) {
			...
			switch (chNum) {
			case 1:
				ch = new PicaChu(); //1번에 해당하ㅏ는 피카츄 객체 생성
				ch.contunue_state(state);
				break;
			case 2:
				ch = new kkobugi(); //1번에 해당하ㅏ는 피카츄 객체 생성
				ch.contunue_state(state);
				break;
			case 3:
				ch = new Ieesanghasi();
			...
			if(ch == null) {
				...
				pg = new PlayGame(ch); // 게임 실행을 위한 객체 생성 후 선택한 캐릭터 전달
			}
