...
//						Member regInfo = getRegInfo();
					...
					case 5:
						// 데이터베이스에서 데이터를 리턴받아
						// 프린트 기능 메서드를 정의하여 출력
						Member[] user_arr = websvc.user_list();
						print_arr(user_arr);
						break;
					case 6:
	...
	private void print_arr(Member[] user_arr) {
		for (int i = 0; i < user_arr.length; i++) {
			System.out.print("email : " + user_arr[i].getEmail() + "\t");
			System.out.print("pwd : " + user_arr[i].getPwd() + "\t");
			System.out.print("Nickname : " + user_arr[i].getNickname() + "\t");
			System.out.println("GRADE : " + user_arr[i].getGrade() + "\t");
